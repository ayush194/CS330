...
void setup_gdt_tss(struct IDTR * gdt)
{
     ...
     while(total_gdts){
          printk("base = %x ",gdt_e->base_low | ((gdt_e->base_mid) << 16) | ((gdt_e->base_high) << 24));
          printk("limit low = %x high=%x ac_byte = %x flags=%x\n", gdt_e->limit_low, gdt_e->limit_high, gdt_e->ac_byte, gdt_e->flags);
          gdt_e->limit_low = 0xffff;
          gdt_e++;
          --total_gdts;
     }
    
     printk("Initializing user segments\n");
...
int do_gp (u64 error, u64 rip)
{
    ...
    printk("General protection fault @ RIP = %x error=%x UserVA=%x\n", rip, error, cr2);
    do_exit();
