...
long  do_syscall(int syscall, u64 param1, u64 param2, u64 param3, u64 param4)
{
    ...
    switch(syscall)
    {
          ...
          case SYSCALL_FORK:
                              return do_fork();
          case SYSCALL_STATS:
                             dprintk("ticks = %d swapper_invocations = %d context_switches = %d lw_context_switches = %d\n", 
                             stats->ticks, stats->swapper_invocations, stats->context_switches, stats->lw_context_switches);
                             dprintk("syscalls = %d page_faults = %d used_memory = %d num_processes = %d\n",
                             stats->syscalls, stats->page_faults, stats->used_memory, stats->num_processes);
                             return stats->file_objects;
                             break;
...
sig_exit:
  printk("PF Error @ [RIP: %x] [accessed VA: %x] [error code: %x]\n", rip, cr2, error_code);
