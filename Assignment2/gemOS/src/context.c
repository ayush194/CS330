...
struct exec_context *get_new_ctx()
{
  ...
  dprintk("%s: System limit on processes reached\n", __func__);
  return NULL;
}
...
static void copy_os_pts(u64 src, u64 dst)
{
   int i;
   ...
   for(ctr=0; ctr < count; ++ctr){
         if(!ctr){
             pfn = start_pfn;
               ...
               if(last_pfn != pfn - 1)
                      dprintk("BUG! PFN not in sequence\n");
         }
...
int exec_init(struct exec_context *ctx, struct init_args *args)
{
   ...
   dprintk("Setting up init process ...\n");
   ...
   dprintk("Page table setup done, launching init ...\n");
...
void setup_child_context(struct exec_context *child)
{
   ...
   do_file_fork(child);
   // Incrementing the file object reference count
   stats->num_processes++;
}
